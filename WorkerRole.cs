    ...
    public class WorkerRole : RoleEntryPoint
    {
        ...
        public void HandleCrawling()
        {
            try
            {
                ...
                if (!crawledUrls.Contains(htmlUrl))
                {
                    ...
                    foreach (HtmlNode dateNode in doc.DocumentNode.SelectNodes("//meta//@name"))
                    {
                        try
                        {
                            if (dateNode.Attributes["name"].Value == "pubdate")
                            {
                                date = "Published date: " + dateNode.Attributes["content"].Value;
                                break;
                            }
                        }
                        catch { }
                    ...
                    InsertTable(htmlUrl, title, date);
        ...
        public void InsertTable(string url, string title, string date)
        {
            try
            {
                ...
                char[] delimiters = {' ', '\t', '-', ',', ':', ';', '!', '?', '\'', '&' };
                string[] words = title.Trim().Split(delimiters);
                foreach (string word in words)
                {
                    if (word.Length > 0)
                    {
                        Page page = new Page(url, title, word.Trim(), date);
